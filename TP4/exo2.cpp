...
void HuffmanNode::processCodes(std::string baseCode)
{
    ...
    {
        this->left->processCodes(baseCode = "O"); 
...
void huffmanHeap(Array& frequences, HuffmanHeap& heap, int& heapSize)
{
    ...
    {
        if(frequneces.get(i)>0)
        {
            heap.insertHeapNode(heapSize, i, frequences.get(i));
            heapSize++; 
        }
    }
}
...
string huffmanEncode(HuffmanNode** characters, string toEncode)
{
    ...
    {
        int ascii = (int)toEncode.at(i); 
        encoded+characters.get(ascii)->code; 
    }
    return encoded;
}
